  ...
  const publishSurvey = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/surveys`, {
        ...
        body: JSON.stringify({
          surveyData,
          surveyId: loadedSurveyId // Will be null for new surveys
        })
      });
        ...
        (surveys || []).map((survey) => (
          ...
          >
            <div 
              ...
              onClick={() => onLoadSurvey(survey.id)}  // Changed from survey.filename
            >
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onDeleteSurvey(survey.id);  // Changed from survey.filename
              }}
              title="Delete survey"
            >
              🗑️
...
const SurveySettingsEditor = ({ 
  ...
  onDescriptionChange,
  onLanguageChange,
  loadedSurveyId  // Add this prop
}) => {
  const [copiedUrl, setCopiedUrl] = React.useState(false);
  
  const formUrl = loadedSurveyId 
    ? `${window.location.origin}/forms/${loadedSurveyId}`
    : null;
  ...
  return (
    <div className="card">
      ...
      <div className="form-group-container">
        ...
        {/* Published Form URL Section */}
        {formUrl && (
          <div className="form-group">
            <label className="label">Published Form URL</label>
            ...
            <div className="help-text">
              💡 Share this URL with respondents to collect survey responses
            </div>
          </div>
        )}
      </div>
